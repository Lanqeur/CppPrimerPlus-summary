...
#### 成员初始化列表
对于const数据成员，必须在执行到构造函数体之前，即创建对象时进行初始化。C++提供了成员初始化列表(member initializer list)。成员初始化列表由逗号分隔的初始化列表组成，前面带冒号。它位于参数列表的右括号后，函数体左括号前。
...
```
从概念上说，初始化工作是在对象创建时完成的，此时还未执行括号中的任何代码。需要注意的是：
* 这种格式只能用于构造函数；
* 必须用这种格式来初始化非静态const数据成员（C++11之前）；
* 必须用这种格式来初始化引用数据成员。

因为引用与const数据都只能在创建时进行初始化。对于简单数据成员，使用成员初始化列表和在函数体中赋值没有什么区别。
...
```
这与在构造函数中使用成员初始化列表等价，成员mem1和mem2将分别被初始化为10和20，除非调用使用成员初始化列表的构造函数，在这种情况下，实际列表将覆盖这些默认初始值：
```Classy::Classy(int n):mem1(n){...}```
这里构造函数将用n初始化mem1，mem2仍然设置为20.
#### 入队方法的实现
将项目添加到队尾，方法实现需要以下几个阶段：
1. 如果队列已满，则结束，返回false；
2. 创建一个新节点。如果new无法创建新节点，将会引发异常，最终的结果是，除非提供了处理异常的代码，否则程序将终止。
3. 在节点中放入正确的值。将Item值复制到节点的数据部分，并将节点的next指针设置为NULL。
4. 将项目计数items++；
5. 将节点附加在队尾。首先，将节点与列表的另一个节点连接起来，就是将当前队尾节点的next指针指向新的队尾节点；然后，将Queue的成员指针rear设置为指向新节点即可。如果队列为空，还要将front指针设置为指向新节点，因为只有一个节点时它同时是队首和队尾节点。
#### 出队方法的实现
1. 如果队列为空，则结束，返回false；
2. 将队列的第一个项目提供给调用函数，即将当前front节点中的数据部分复制到传递给方法的引用变量。
3. 项目计数items--；
4. 保存front节点的位置，供以后删除；
5. 让节点出队，即将front指针设置成指向下一个节点来完成的。
6. 删除以前的第一个节点。
7. 如果链表为空，则将rear设置为NULL。
#### 其它类方法的需求
* 因为向队列中添加对象将调用new来创建新的节点，因此类需要一个显式的析构函数来删除所有节点。
* 要克隆或者复制队列，必须提供复制构造函数和执行深度复制的赋值构造函数。
* 不需要复制功能时，可以将复制构造函数和赋值运算符定义为私有方法。这样可以避免自动生成的默认方法定义。C++11提供了使用关键字delete禁用方法的方式。
* 还需要注意的是，当对象被按值传递或返回时会调用复制构造函数，如果按引用传递则不会有任何问题。复制构造函数还会用于创建临时对象，但Queue类中没有导致创建临时对象的操作，例如重载加法运算符。
## ATM模拟
程序允许用户输入3个数：队列的最大长度，程序模拟持续的时间单位：小时，以及平均每小时的客户数。程序将使用循环，每个循环的间隔是1分钟，在每个循环中，程序将完成如下工作：
1. 判断是否有新客户。如果队列未满，把它加入队尾，否则拒绝客户入队。
2. 如果没有客户在交易，则选取队列第一个客户，确定该客户的等待时间，并将wait_time计数器设置为新客户所需的处理时间。
3. 如果客户正在处理中，则将计数器减1.
4. 记录各种数据，如获得服务的客户数目，被拒绝的客户数目，排队等候的累积时间，以及累积的队列的长度等。
5. 当模拟循环结束时，程序将报告各种统计结果。

假设平均每小时来10个客户，那么大约6分钟会来一名客户，为了让客户到达时间具有随机性，且平均时间接近6分钟，可以使用如下的函数：
 ```C++
